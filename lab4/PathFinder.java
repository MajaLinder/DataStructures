...
public class PathFinder<Node> {
    ...
    public Result searchAstar(Node start, Node goal) {
        ...
        Queue<PQEntry> pqueue = new PriorityQueue<>(Comparator.comparingDouble(entry -> entry.costToHere + entry.costStartToGoal));
        ...
        while(!pqueue.isEmpty()) {
            ...
            if (!visitedNodes.contains(entry.node)) {
                ...
                for (DirectedEdge<Node> edge : graph.outgoingEdges(entry.node)) {
                    double costToNext = entry.costToHere + edge.weight();
                    pqueue.add(new PQEntry(edge.to(), costToNext, entry, graph.guessCost(edge.to(), goal)));
                }
            }
        }
        return new Result(false, start, goal, -1, null, iterations);
    ...
    private class PQEntry {
        ...
        public final double costStartToGoal;

        PQEntry(Node n, double c, PQEntry bp, double cs) {
            node = n;
            costToHere = c;
            backPointer = bp;
            costStartToGoal = cs;
